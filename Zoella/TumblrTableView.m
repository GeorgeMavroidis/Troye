...
@implementation TumblrTableView
...
-(void)fetchFeeds{
    ...
    NSString *posts_url = [NSString stringWithFormat:@"http://thewotimes.com/Y/current.php?user=%@&type=tumblr&get=true", USER];
    ...
    nextPageToken = [NSString stringWithFormat:@"%@",[[[items objectAtIndex:[items count]-1] objectForKey:@"entry"] objectForKey:@"id"]];
    //    items = json_dic;
    
    NSUserDefaults *d = [NSUserDefaults standardUserDefaults];
    [d setObject:tdata forKey:@"tumblr"];
...
-(void)ajaxFeeds{
    ...
    fetching = YES;
    NSString *posts_url = [NSString stringWithFormat:@"http://thewotimes.com/Y/user.php?user=%@&type=tumblr&next=%@", USER, nextPageToken];
    NSURL *url = [NSURL URLWithString:posts_url];
    NSError* error;
    NSString *t = [NSString stringWithContentsOfURL:url usedEncoding:nil error:&error];
    NSData *tdata = [t dataUsingEncoding:NSUTF8StringEncoding];
...
-(void)loadFeedFor:(NSString *)page{
    ...
    NSUserDefaults *d = [NSUserDefaults standardUserDefaults];
    items = [NSJSONSerialization JSONObjectWithData:[d objectForKey:@"tumblr"] options:0 error:nil];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    return [CreationFunctions tableView:tableView heightForTumblr:indexPath singleton:items];
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell = [CreationFunctions createTumblrCell:tableView cellForRowAtIndexPath:indexPath singleton:items];
